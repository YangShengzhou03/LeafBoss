...
![LEAF-BOSS Overview](https://gitee.com/Yangshengzhou/leaf-boss/raw/master/assets/LEAF-BOSS-Overview.png)

![LEAF-BOSS Login](https://gitee.com/Yangshengzhou/leaf-boss/raw/master/assets/LEAF-BOSS-Login.png)

## Quick Start

### System Requirements

Before using LeafBoss, ensure your development environment meets these requirements: Node.js 16.0+, Java 17.0+, MySQL 8.0+, and Maven 3.6+. For production, a Linux server with at least 2GB RAM and 1GB storage is recommended.

### Installation & Deployment

1. **Clone Project**: `git clone https://github.com/YangShengzhou03/LeafBoss.git`
2. **Backend Setup**: Create a MySQL database named `leaf_boss`, execute `data.sql`, configure `application.yml` with your DB and JWT settings, then run `mvn clean package` and start the JAR.
3. **Frontend Setup**: In the `frontend` directory, run `npm install`, configure `.env.development`, and start with `npm run serve` (dev) or `npm run build` (prod).

### Quick Usage

Access the frontend at http://localhost:8080 and backend API at http://localhost:8081/api.
**Default Admin**: Email `admin@qq.com`, Password `123456`.

After logging in, administrators can access:
- **Card Key Management**: Batch generate codes, verify status, manage lifecycle, and export to TXT.
- **Product Management**: Maintain product lines and configure multiple authorization specs (price, duration, stock).
- **Company & Reviews**: Manage company profiles and audit user reviews.
- **Personnel Management**: Manage system admins (CRUD, status lock, password reset) and customer users.
- **Audit Logs**: Track sensitive operations by time, type, and IP.
- **Dashboard**: Real-time overview of inventory, daily sales, and revenue trends.

![LEAF-BOSS Dashboard](https://gitee.com/Yangshengzhou/leaf-boss/raw/master/assets/LEAF-BOSS-Dashboard.png)
