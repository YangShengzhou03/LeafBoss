...
public class CardKeyController {
    ...
    public Result<Map<String, Object>> verifyCardKey(@PathVariable String cardKey) {
        ...
        if (card != null) {
            ...
            return Result.error(404, "卡密不存在，请检查卡密是否正确");
        }
    }
    ...
    public Result<Boolean> activateCardKey(@RequestBody Map<String, String> requestBody, HttpServletRequest request) {
        ...
        if (updated) {
            String productSpecName = getProductSpecName(cardKeyEntity);
    ...
    public Result<Boolean> deleteCardKeyById(@PathVariable String id, HttpServletRequest request) {
        ...
        if (deleted) {
            String productSpecName = getProductSpecName(cardKey);
    ...
    public Result<Boolean> toggleCardKeyStatus(
        ...
        if (card != null) {
            ...
            if (updated) {
                String productSpecName = getProductSpecName(card);
                String action = "启用";
    ...
    // 获取卡密对应的商品规格名称，用于日志记录
    private String getProductSpecName(CardKey cardKey) {
        ...
        return productName + "-" + spec.getName();
    }
}
