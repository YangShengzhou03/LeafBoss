...
# 将证书leafboss.top_certificate.pem和私钥文件leafboss.top_private.key复制到/data/nginx/certs/目录下
cd /data/nginx/certs/
docker run -d \
...
- 访问方式：仅支持 `https://你的域名`（如 `https://leafboss.top`），`http://服务器IP` 会自动跳转到 HTTPS；
